...
class Sorter (object):
    ...
    def __init__(self, motor_belt, motor_horizontal, motor_coin, video_url):
        ...
        self.coins_size = {82: '0.1', 99: '0.2', 104: '0.5', 120: '1', 115: '2', 125: '5', -1: 'ERROR'}

        cv2.namedWindow("preview")
        self.vc = cv2.VideoCapture(video_url)
        self.dequeue = collections.deque(maxlen=59)
    ...
    def run_automata(self):
        ...
        while True:
            self.state_1()
            self.state_2()
            self.computed_position = self.state_3()
            if len(self.dequeue) == 0:
                continue
            self.state_4(self.computed_position)
            self.state_5()

    def state_1(self, power=-65, turn=200, isbreak=True):
        # Get a coin
        self.push_coin()
    ...
    def push_coin(self):
            self.motor_coin.turn(15, 360, brake=False)

    def move_coin_to_camera(self, power=30, turn=35, isbreak=False):
    ...
    def state_2(self):
       ...
       while True and self.counter < self.frame_counter:
            ...
            gauss_blur = cv2.GaussianBlur(cimg, (13, 13), -1)

            cv2.imshow('grayscale', gauss_blur)
            # _, binary_frame = cv2.threshold(gauss_blur, 50, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            circles = cv2.HoughCircles(gauss_blur, cv2.HOUGH_GRADIENT, 1.2, 300,
                                                param1=100, param2=35, minRadius=80, maxRadius=135)
            ...
            if circles is not None:
                ...
                for i in circles[0, :]:
                    ...
                    print("coin size: " " " + str(self.coins_size[min(self.coins_size, key=lambda x: abs(x-i[2]))]))
                    self.dequeue.append(i[2])
                    print("i: ", i[2])
                    # draw the outer circle
                    cv2.circle(frame, (i[0], i[1]), i[2], (0, 255, 0), 2)
                    # draw the of the circle
    ...
    def state_3(self):
        ...
        position = self.coins_box_posistion.get(str(coin_type), -1) 
    ...
    def state_4(self, new_position):
        ...
        self.to_position = int(new_position) - int(self.current_horizontal_position)
    ...
    def state_5(self, power=65, turn=400, isbreak=False):
        """Get of coin"""
        self.belt.turn(power, turn, isbreak)
...
#motor_2.turn(25, 360*50, brake=True)

#motor_1 = None
#motor_2 = None
#motor_3 = None

sorter = Sorter(motor_1, motor_3, motor_2, 'http://192.168.42.11:4747/mjpegfeed?640x480')
sorter.run_automata()
